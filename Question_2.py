...
def main():
    ...
    # 1) Seasonal averages (overall)
    write_seasonal_averages(df, OUTPUT_AVG)

    # 2) Largest temperature range station(s)
    write_largest_range(df, OUTPUT_RANGE)

    # 3) Temperature stability (std dev)
    write_stability(df, OUTPUT_STABILITY)

    # Optional: short console summary
    print(f"Files processed from: {DATA_DIR}")
    print(f"Unique stations: {df['STATION_NAME'].nunique()}")
    print(f"Data points: {len(df)}")
    print(f"Wrote: {OUTPUT_AVG}, {OUTPUT_RANGE}, {OUTPUT_STABILITY}")
